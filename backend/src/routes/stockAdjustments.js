...
router.get('/', authenticateToken, async (req, res) => {
    try {
        const { page = 1, limit = 10, product_id = '', adjustment_type = '', user_id = '' } = req.query;
        const offset = (page - 1) * limit;

        let whereConditions = ['sm.movement_type = "adjustment"'];
        let queryParams = [];
        ...
        // Kullanıcı filtresi
        if (user_id) {
            whereConditions.push('sm.created_by = ?');
        ...
        const [adjustments] = await db.pool.execute(query, queryParams);
    ...
    try {
        ...
        if (adjustment_type === 'decrease') {
            ...
            if (availableQuantity < quantity) {
                return res.status(400).json({
                    success: false,
                    message: `Yetersiz stok. Mevcut: ${availableQuantity} ${product[0].unit}, Düzeltilecek: ${quantity} ${product[0].unit}`
                });
            }
        }

        // Düzeltme numarası oluştur
        const adjustmentNumber = `ADJ-${Date.now()}-${Math.random().toString(36).substr(2, 4).toUpperCase()}`;

        // Stok hareketini kaydet (artış için pozitif, azalış için negatif miktar)
        const movementQuantity = adjustment_type === 'increase' ? quantity : -quantity;
        ...
        `, [
            ...
            movementQuantity,
            unit_cost || null,
            `${adjustmentNumber} - ${reason}: ${notes || 'Manuel stok düzeltmesi'}`,
            req.user.id
        ]);
        ...
        const [newAdjustment] = await connection.execute(`
            SELECT 
                sm.id,
                sm.uuid,
                sm.product_id,
            ...
            FROM stock_movements sm
            LEFT JOIN products p ON sm.product_id = p.id
            WHERE sm.product_id = ? AND sm.created_by = ? AND sm.notes LIKE ?
            ORDER BY sm.created_at DESC
            LIMIT 1
...
router.get('/stats/summary', authenticateToken, async (req, res) => {
    try {
        ...
        const [topProducts] = await db.pool.execute(`
            SELECT 
                p.name as product_name,
                p.sku,
        ...
        res.json({
            ...
            data: {
                summary: stats[0],
                top_adjusted_products: topProducts
            }
        });
    ...
    try {
        ...
        for (let i = 0; i < adjustments.length; i++) {
            ...
            try {
                ...
                if (!adjustment.product_id || !adjustment.adjustment_type || !adjustment.quantity || !adjustment.reason) {
                ...
                if (product.length === 0) {
                    errors.push(`Satır ${i + 1}: Ürün bulunamadı (ID: ${adjustment.product_id})`);
                    continue;
                }
                ...
                const movementQuantity = adjustment.adjustment_type === 'increase' ? adjustment.quantity : -adjustment.quantity;
                ...
                `, [
                    ...
                    `${adjustmentNumber} - ${adjustment.reason}: ${adjustment.notes || 'Toplu stok düzeltmesi'}`,
                    req.user.id
                ]);
