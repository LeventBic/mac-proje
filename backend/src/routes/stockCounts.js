  ...
  async (req, res) => {
    try {
      ...
      if (!scheduled_date) {
        return res
          .status(400)
          .json({ success: false, message: 'Planlanan tarih zorunludur' })
      }
      const countNumber = `COUNT-${Date.now()}-${Math.random()
        .toString(36)
        .substr(2, 4)
        .toUpperCase()}`
      const result = await query(
        `
            INSERT INTO stock_counts (count_number, location_id, scheduled_date, status, notes, created_by)
            VALUES (
              $1,
              (SELECT id FROM locations WHERE code = $2 LIMIT 1),
              $3,
              'planned',
              $4,
              $5
      ...
      res
        ...
        .json({
          ...
          data: { id: result.rows[0].id, count_number: countNumber }
        })
    } catch (error) {
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: 'Sayım bulunamadı' })
    }
  ...
  async (req, res) => {
    try {
      ...
      if (counted_quantity === undefined) {
        return res
          .status(400)
          .json({ success: false, message: 'Sayım miktarı zorunludur' })
      }
      await query(
        `
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        data: { total_items: products.length }
      })
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        data: { discrepancies: discrepancies.diff_count }
      })
    } catch (error) {
